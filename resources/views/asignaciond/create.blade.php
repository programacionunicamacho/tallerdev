...
{!!Form::open(array('url'=>'asignaciond','method'=>'POST','autocomplete'=>'off'))!!} {{Form::token()}} <div
    class="row">
    <div class="col-lg-4 col-md-9 col-sm-6 col-xs-12">
        <div class="form-group"> <label for="personas_id">Docente</label>
    ...
    <div class="col-lg-4 col-md-9 col-sm-6 col-xs-12">
        <div class="form-group"> <label for="asignaturas_id">Asignatura</label> 
        <select name="asignaturas_id"  id="asignaturas_id" class="form-control selectpicker" data-livesearch="true" required>
                <option value="" disabled selected>Asignatura:</option>
                 @foreach($asignaturas as $asignatura) <option
                    value="{{$asignatura->id}}">{{ $asignatura->codigo_asignatura }} - {{$asignatura->nombre_asignatura }}</option>
                     @endforeach
            </select>
         </div>
    </div>
             ...
             <label for="cursos_id">Cursos:</label>
              <select name="cursos_id" id="cursos_id"  class="form-control selectpicker" data-livesearch="true" required>
                <option value="" disabled selected>Seleccione el Curso:</option>
                 ...
                 <option   value="{{$curso->id}}">{{ $curso->curso }} - {{$curso->nombre}}</option> @endforeach
            </select> </div>
    </div>
    <div class="col-lg-4 col-md-9 col-sm-6 col-xs-12">
        <div class="form-group"> <label for="ano_lectivo">Año Lectivo</label> 
        <input type="text" name="ano_lectivo" id="ano_lectivo" class="form-control" placeholder="Año lectivo"> </div>
    </div>
    <div class="col-lg-6 col-md-12 col-sm-6 col-xs-12">
        <div class="form-group"> <br> <button class="btn btn-primary" type="submit"><span
            ...
            <button class="btn btn-danger" type="reset"><span class="glyphicon glyphicon-remove"></span> Vaciar
                Campos</button> <a class="btn btn-info" type="reset" href="{{url('asignaciond')}}"><span
